...
fn main() {
    ...
    println!(
        "  {:<12} {:>14} {:>14} {:>12}",
        "returns", "per-node adv", "top traffic", "herfindahl"
    );
    ...
    println!(
        "  \x1b[2mThe ceiling does what it claims: with equal uptime, no per-node edge.\x1b[0m"
    );
    ...
    println!(
        "  {:<24} {:>14} {:>14}",
        "giant uptime", "per-node adv", "top traffic"
    );
    ...
    println!(
        "  \x1b[2mThe hypothesis was that uptime would route around the ceiling. It does\x1b[0m"
    );
    println!("  \x1b[2mnot: standing per node stays flat across the whole range, because a\x1b[0m");
    println!(
        "  \x1b[2mceiling is a ceiling however often you reach it. Reliability buys a few\x1b[0m"
    );
    ...
    println!("{}", "-".repeat(72));
    for (label, t) in [
        ("transferable standing", true),
        ("KARST: non-transferable", false),
    ] {
        let a = acquire(10.0, 40.0, t);
        println!(
            "  {:<26} buyer {:.0} + seller {:.0} -> {:.0}  (gain {:.0})",
            label,
            a.buyer_before,
            a.seller,
            a.buyer_after,
    ...
    println!(
        "  {:<22} {:>18} {:>20}",
        "fleet size", "paths touched", "both endpoints held"
    );
    ...
    println!("{}", "-".repeat(72));
    println!(
        "  \x1b[1mClaims 1 and 2 hold.\x1b[0m Standing per node stays at 1.00 to 1.01 across a"
    );
    ...
    println!();
    println!(
        "  \x1b[1mThe hole is observation, and it is not small.\x1b[0m An adversary who wants"
    );
    ...
    println!();
    println!(
        "  \x1b[1mVerdict: L16 raises the cost of buying position and does nothing about\x1b[0m"
    );
